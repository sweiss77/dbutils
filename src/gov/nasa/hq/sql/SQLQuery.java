...
public class SQLQuery implements java.io.Serializable {
    ...
    public SQLQuery( SelectClause select, FromClause from,
                     WhereClause where, AndClause and ) {
        init( select, from, where, and, null, null, null );
    }

    public SQLQuery( SelectClause select, FromClause from,
                     WhereClause where, OrClause or ) {
    ...
    public SQLQuery( String[] cols, String[] tables ) {
        init( new SelectClause( cols ), new FromClause( tables ),
              null, null, null, null, null );
    }
    ...
    public SQLQuery( String[] cols, String table, WhereClause where ) {
        init( new SelectClause( cols ), new FromClause( table ),
              where, null, null, null, null );
    }

    void init( SelectClause select, FromClause from, WhereClause where,
               AndClause and, OrClause or, OrderByClause orderBy,
        ...
        if ( and != null ) _and = and;
        if ( or != null ) _or = or;
        if ( orderBy != null ) _orderBy = orderBy;
        if ( groupBy != null ) _groupBy = groupBy;
    ...
    public String toString() {
        ...
        if ( _clauses != null ) {
            ...
            for ( int i = 0; i < _clauses.size(); i++ ) {
                ...
                if ( i < _clauses.size() -1 ) work.append( ' ' );
            }
